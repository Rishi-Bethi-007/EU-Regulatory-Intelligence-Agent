...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    Promise.all([fetchStats(), fetchDocs()]).finally(() => setLoading(false))
  }, [])
  ...
  const TYPE_LABEL: Record<string, string> = {
    ...
    swedish_market: '🇸🇪 Swedish Market',
    german_market: '🇩🇪 German Market',
  }

  return (
    <div className="p-8 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-white mb-2">📄 Regulatory Knowledge Base</h1>
      <p className="text-gray-400 text-sm mb-8">
        Curated EU AI Act, GDPR, and national regulatory guidance documents in English, Swedish, and German.
        Embedded with <span className="text-blue-400 font-mono">multilingual-e5-large</span> (1024 dims) · Hybrid dense+sparse retrieval.
      </p>
        ...
        <>
          ...
          {stats && stats.total_chunks > 0 && (
            <div className="card mb-8">
              <h2 className="text-sm font-semibold text-gray-400 mb-3">Language distribution</h2>
              {[
                { label: '🇬🇧 English', count: stats.en,  color: 'bg-blue-600'  },
                { label: '🇸🇪 Swedish', count: stats.sv,  color: 'bg-yellow-500' },
                { label: '🇩🇪 German',  count: stats.de,  color: 'bg-red-600'    },
                ...
                <div key={label} className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1 h-2 bg-gray-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(count / stats.total_chunks * 100).toFixed(1)}%` }}
                    />
                  </div>
                  <span className="text-xs text-gray-500 w-12 text-right">
                    {(count / stats.total_chunks * 100).toFixed(1)}%
                  </span>
                </div>
              ))}
            </div>
          ...
          {/* Document table */}
          <div className="card">
            <h2 className="text-sm font-semibold text-gray-400 mb-4">Corpus Documents ({docs.length})</h2>
                ...
                <div key={doc.id} className="flex items-center justify-between py-2 border-b border-gray-800 last:border-0">
                  <div className="flex-1 min-w-0">
                    {doc.source_url ? (
                      ...
                      <span className="text-sm text-gray-200 font-medium">{doc.title}</span>
                    )}
                  </div>
            ...
            <p className="text-xs text-gray-600 mt-4">
              Documents are ingested by the system administrator via the ingestion pipeline.
            </p>
          </div>
        </>
      )}
