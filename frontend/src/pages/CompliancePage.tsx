...
export default function CompliancePage() {
  ...
  async function handleAccessData() {
    ...
    try {
      ...
      setGdprData(data)
    } catch (e: any) { setGdprMsg(`Error: ${e.message}`) }
        ...
        {!activeRunId ? (
          ...
          <>
            {run?.goal && !run.goal.startsWith('[ERASED') && (
              <p className="text-gray-600 text-sm mb-4 line-clamp-2">{run.goal}</p>
            )}
                ...
                <div className="card">
                  ...
                  <div className="space-y-2 mt-2">
                    {SCORE_DIMENSIONS.map(({ key, label, desc }, i) => {
                      const passed = breakdown ? breakdown[key]?.passed : score >= (i + 1) * 20
                      return (
            ...
            {tab === 'gdpr' && (
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="card border-red-200">
                  <h3 className="text-sm font-semibold text-gray-900 mb-1">🗑️ Art. 17 — Right to Erasure</h3>
