...
export default function Layout() {
  ...
  return (
    <div className="flex min-h-screen bg-transparent">
      {mobileOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 z-20 md:hidden" onClick={() => setMobileOpen(false)} />
      )}

      <aside className={`fixed h-full z-30 flex flex-col bg-white border-r border-gray-200 transition-all duration-200 ${sidebarW} ${mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-2 space-y-0.5">
            ...
            <NavLink key={to} to={to} end={to === '/app'} onClick={() => setMobileOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  isActive ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                }`}>
          ...
          {isAdmin && (
            <NavLink to="/app/admin" onClick={() => setMobileOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                  isActive ? 'bg-red-50 text-red-700 border border-red-100' : 'text-red-500 hover:text-red-700 hover:bg-red-50'
                }`}>
        ...
        <div className="p-2 border-t border-gray-100">
          {!collapsed && <div className="text-xs text-gray-600 truncate mb-2 px-1">{user?.email}</div>}
          <button onClick={handleSignOut}
            className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-700 hover:text-gray-900 hover:bg-gray-50 transition-colors"
            title="Sign out">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-20 bg-white border-b border-gray-200 flex items-center px-3 py-2 gap-3">
        <button onClick={() => setMobileOpen(o => !o)} className="p-2 rounded text-gray-700 hover:text-gray-900 hover:bg-gray-100 transition-colors">☰</button>
